...
A latest user message marked \`delivery="while-active"\` is an interjection during active work, not a checkpoint by itself. Answer ordinary status questions or clarifications, then continue autonomous goal work when the goal is unfinished. Do not continue when the message explicitly asks to stop, pause, or cancel, or when a checkpoint required by the goal is still outstanding.
Use "continue" when the goal is not done and the assistant should keep working autonomously, including after answering a while-active status question and when it asked for input that the goal did not explicitly require.
